...
export default function ReceiptsPage({
  ...
  const {
    ...
    queryKey: queryKeys.finance.payments(clinicSlug, "successful"),
    queryFn: () => getPayments(clinicSlug, "successful"),
    enabled: !!clinicSlug,
  });
  ...
  return (
    <div className="space-y-4">
      <h1 className="flex items-center gap-2 text-xl font-bold text-gray-900 dark:text-white">
        <Wallet className="h-5 w-5 text-primary-dark dark:text-primary-light" /> دریافت‌ها
      </h1>

      <div className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <div className="text-[11px] text-gray-400 dark:text-gray-500">مجموع دریافت‌های موفق</div>
        <div className="mt-1 text-lg font-bold text-primary-dark dark:text-primary-light">
          {formatToman(total)}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        {isLoading ? (
          ...
          <p className="py-10 text-center text-xs text-gray-400 dark:text-gray-500">دریافتی ثبت نشده است.</p>
        ) : (
