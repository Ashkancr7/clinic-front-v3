...
async function forward(
   ...
   * =========================================================
   *
   * برای آپلود فایل (multipart/form-data)، Content-Type اصلی
   * (به‌همراه boundary) باید دست‌نخورده عبور کند و بدنه به‌صورت
   * باینری خوانده شود؛ در غیر این صورت فایل آپلودی خراب می‌شود.
   */
  ...
  try {
    upstream = await fetch(targetUrl, {
      method,
      headers,
      body: hasBody ? (isMultipart ? await req.arrayBuffer() : await req.text()) : undefined,
      cache: "no-store",
    });
  } catch (error) {
    console.error(
